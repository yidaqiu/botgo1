...
func (p Processor) ProcessMessage(input string, data *dto.WSATMessageData) error {
	...
	case "/猜灯谜":
		toCreate.Content = guess.GetRiddle()
		p.sendReply(ctx, data.ChannelID, toCreate)
	case "/谜底":
		if cmd.Content == "" {
			toCreate.Content = "请输入谜语序号"
			p.sendReply(ctx, data.ChannelID, toCreate)
			return nil
		}

		id, err := strconv.Atoi(cmd.Content)
		if err != nil {
			toCreate.Content = guess.GetAnswerByID(id).Answer
		} else {
			toCreate.Content = guess.GetAnswerByID(id).Answer
		}
